...
investimentos_marketing_organizada <- investimentos_marketing %>%
  pivot_longer(
    cols = c("Jan":"Mar"), 
    names_to = "mês",
    values_to = "resultado" # nome de outra nova coluna
  )
...
```{r}
# Transforman para o formato longo/organizado
indicadores_dptos_organizada <- indicadores_dptos %>%
  pivot_longer(
    cols = -departamento,
    names_to = c("mês", "indicador"), 
    names_sep = "_", 
    values_to = "valor" 
  )

# Visualiza o resultado
indicadores_dptos_organizada
```
...
paises_economia <- gapminder %>%
  ...
  select(country, economia_grande) %>%
  head(6)
...
```{r}
expect_vida_classificados <- gapminder %>%
  filter(year == 2007) %>%
  mutate(
    expectativa_vida = case_when(
      lifeExp < 60 ~ "Baixa",
      lifeExp >= 60 & lifeExp <= 75 ~ "Médio",
      lifeExp > 75 ~ "Alto"
    )
  )%>%
  select(country, continent, lifeExp, expectativa_vida)
expect_vida_classificados
...
```{r}
pais_filial <- gapminder %>%
  group_by(continent) %>%
    summarize(numero_filiais = n_distinct(country),
...
ranking_continentes <- gapminder %>%
  filter(year == 2007) %>%
  group_by(continent) %>%
    summarize(exp_vida_media = mean(lifeExp)) %>%
    arrange(desc(exp_vida_media))
